    ...
    def __str__(self):
        if len(self.links) >= 1:
            ...
            for i in range(0, len(self.links)):
                string += ' ' + 'INNER JOIN ' + str(self.links[i][1][0]) + ' ' + 'ON ' + str(self.links[i][0][0]) + '.' + str(
    ...
    def __str__(self):
        ...
        if len(self.conditions) >= 1:
            for i in range(0, len(self.conditions)):
                if i == 0:
                    string += ' ' + color.BOLD + 'WHERE' + \
                        color.END + ' ' + str(self.conditions[i][1])
                else:
                    string += ' ' + color.BOLD + \
                        str(self.conditions[i][0]) + color.END + \
                        ' ' + str(self.conditions[i][1])
            return string
        else:
            return string
    ...
    def print_json(self, output):
        if len(self.columns) >= 1:
            if len(self.columns) == 1:
                ...
                output.write(
                    '\t\t"column": "' + self.get_just_column_name(str(self.columns[0])) + '"\n')
                output.write('\t},\n')
            else:
                output.write('\t"order_by": {\n')
                output.write('\t\t"order": "' + str(self.order) + '",\n')
                ...
                for i in range(0, len(self.columns)):
                    if i == (len(self.columns)-1):
                        output.write(
                            '"' + self.get_just_column_name(str(self.columns[i])) + '"')
                    else:
                        output.write(
                            '"' + self.get_just_column_name(str(self.columns[i])) + '", ')
                output.write(']\n')
